...
void handleLoadImage() {
    ...
    if (currentImage.load(path)) {
        ...
        std::cout << "Failed to load image from " << path << ". Make sure the file exists and is a valid PGM (P2) format.\n";
    }
}
...
int main() {
    int choice;
    do {
        std::cout << "\n--- Image Processing Application Menu ---\n";
        std::cout << "Current Image Status: " << (currentImage.isEmpty() ? "No image loaded" : "Loaded (" + std::to_string(currentImage.width()) + "x" + std::to_string(currentImage.height()) + ")") << "\n";
        ...
        case 0: std::cout << "Exiting application. Goodbye!\n"; break;
